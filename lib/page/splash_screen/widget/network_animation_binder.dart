    ...
    for (int i = 0; i < nodes.length; i++) {
      for (int j = i + 1; j < nodes.length; j++) {
        ...
        if (distance < connectionDistance) {
          ...
          final pulseEffect =
              sin((animationValue * 4 * pi) + (i + j) * 0.2) * 0.2 + 0.8;
          linePaint.color = lineColor.withValues(alpha: opacity * pulseEffect);
          linePaint.strokeWidth =
              1.0 + (1 - distance / connectionDistance) * 1.5;
...
class _NetworkAnimationWidgetState extends State<NetworkAnimationWidget>
  ...
  void _updateNodes() {
    ...
    for (final node in _nodes) {
      ...
      final padding = 20.0;
      if (node.position.dx < padding ||
          node.position.dx > _lastSize.width - padding) {
      ...
      if (node.position.dy < padding ||
          node.position.dy > _lastSize.height - padding) {
        node.velocity = Offset(node.velocity.dx, -node.velocity.dy);
        node.position = Offset(
          node.position.dx,
